...
def get_conversation_context(session_id):
    ...
    # Format the conversation with additional metadata
    context_parts = []
    recent_messages = session_conversations[session_id][-6:]  # Last 6 messages for context
    ...
    context_summary = []
    if lead_data.get("in_qualification"):
        context_summary.append(f"Current Status: In lead qualification process (stage: {lead_data.get('current_question', 'unknown')})")
    if consultation_data.get("in_consultation"):
        context_summary.append(f"Current Status: In consultation request process")
    
    full_context = "\n".join(context_parts)
    if context_summary:
        full_context = f"SESSION STATE: {'; '.join(context_summary)}\n\nCONVERSATION:\n{full_context}"
...
@tool
def clients_reviews(user_message: str, conversation_context: str = "") -> str:
...
IMPORTANT: Keep response to exactly 2 lines maximum. Use the exact phrasing specified above.""",
            input_variables=["user_message", "conversation_context"]
        )
        
        clients_chain = clients_prompt | llm
        response = clients_chain.invoke({
        ...
        return f"We have diverse clients across the world, you can check it out:\nOur Clients -  https://www.genetechsolutions.com/clients"


@tool
def pricing_info(user_message: str, conversation_context: str = "") -> str:
    """Use this tool for pricing-related queries. Returns relevant pricing links in 1-2 lines max."""
    try:
        pricing_prompt = PromptTemplate(
            template=f"""You are {COMPANY_NAME}'s professional AI assistant handling pricing queries.
...
- Reference previous topics naturally when relevant
- Maintain conversational flow and continuity
- For pricing-related questions, provide ONLY the appropriate answer
...
Pricing :
Project based: To be estimated after the Project Requirement Document is shared.
Hourly : Per-hour Rate USD 50
Dedicated Resource : Junior Developer: USD 2500/Month , Mid-level Developer: USD 3000/Month , Senior Developer: USD 4000/Month
        
Match keywords to provide the most relevant pricing information. If unsure, provide general pricing information.
...
@tool
def company_portfolio(user_message: str, conversation_context: str = "") -> str:
...
@tool
def handle_greeting_feedbacks(user_message: str, conversation_context: str = "") -> str:
...
@tool
def handle_irrelevant_queries(user_message: str, conversation_context: str = "") -> str:
...
- For queries NOT directly related to {COMPANY_NAME} or its services:
  ...
  - Keep responses concise (1 sentence max)
        
Example approach for irrelevant queries:
User: "What are ways to make life better?"
Response: "While I focus on {COMPANY_NAME}'s services, many of our clients find life easier with reliable tech solutions. We'd love to show you how our development services can streamline your business—would you like to explore this?"
...
IMPORTANT HARD RULES: Keep all responses concise and to the point. Avoid lengthy explanations - aim for 1 sentence maximum that deliver clear value and encourage action.""",
            input_variables=["user_message", "conversation_context"]
        )
        
        irrelevant_chain = irrelevant_prompt | llm
        response = irrelevant_chain.invoke({
            "user_message": user_message,
            "conversation_context": conversation_context
        })
...
@tool
def looking_job_opportunity(conversation_context: str = "") -> str:
    ...
    try:
        job_prompt = PromptTemplate(
            template=f"""You are {COMPANY_NAME}'s professional AI assistant handling job opportunity inquiries.
...
- Reference previous topics naturally when relevant
- Maintain conversational flow and continuity
- Provide information about job opportunities at {COMPANY_NAME}
...
@tool
def company_contact_info(conversation_context: str = "") -> str:
...
@tool
def search_company_info(question: str, conversation_context: str = "") -> str:
    ...
    try:
        ...
        prompt = PromptTemplate(
            template=f"""You are {COMPANY_NAME}'s professional AI assistant. Respond to customer inquiries with warmth, expertise, and conversational awareness.
...
- Keep answers concise and conversational (1-2 sentences max)
- Use a warm, human-like tone that builds trust
- Use Conversational context to enhance responses
...
Example 1 - Services Inquiry:
User: "What services do you provide?"
Response: "We specialize in comprehensive tech solutions including web development, mobile app development, AI integration, and cybersecurity services to help businesses thrive in the digital landscape. Would you like to schedule a quick call to discuss which of these services would be the perfect fit for your specific project?"


Example 2 - Industry Focus:
User: "What industries do you specialize in?"
Response: "We have extensive experience working with a variety of industries including healthcare, fintech, e-commerce, education and logistics/Supplychain. Our tailored solutions are designed to meet the unique challenges of each sector. Can you share more about your industry and specific needs?"

Example 2 - Technology Stack:
User: "What technologies do you work with?"
Response: "Our expert team works with modern tech stacks including React, Node.js, Python, Flutter, cloud platforms like AWS, and cutting-edge AI frameworks to build scalable, secure solutions. What type of project are you looking to bring to life?"

Example 3 - Company Information:
User: "Tell me about your company"
Response: "We're a passionate team of tech experts dedicated to transforming businesses through innovative digital solutions, with a proven track record of delivering exceptional results. Ready to see how we can elevate your business to the next level?"
...
query_router_agent = Agent(
    ...
    - pricing_inquiry → pricing_info (for pricing, cost, quote inquiries)
    ...
    tools=[handle_greeting_feedbacks, start_lead_qualification, continue_lead_qualification, start_consultation_request, continue_consultation_request, looking_job_opportunity, company_contact_info, search_company_info, handle_irrelevant_queries, company_portfolio, clients_reviews, pricing_info],
    llm=llm,
    verbose=False,
    allow_delegation=False,
    max_iter=5
...
def classify_query_intent(user_input: str, crew, conversation_context: str, session_id: str) -> str:
    ...
    classification_task = Task(
        ...
        3. "consultation_request" - User specifically asks for consultation, wants to contact someone or sales team , asks "how do I contact","how do i contact you", "can I get a quick consultation", "how can I contact you", "wants to speak with team", "how can i contact to sales team ?", "can you please Connect me to the sales team ?", "i need to get touch with sales team. Please Connect me to them"
        4. "company_info" - Questions specifically about Genetech Solutions' services, team, processes,  that require company knowledge Example "do you provide Cybersecurity services?", "What services do you provide"
        ...
        9. "pricing_request" - User asking about pricing, quotes, or cost estimates for services Example "What are your rates?", "How much do you charge?", "Can I get a quote?", "Share your pricing" , "what is pricing for web development?"
        10. "irrelevant" - General knowledge questions not specific to Genetech Solutions
    ...
    try:
        ...
        valid_intents = ['greeting_feedback', 'business_interest', 'consultation_request', 'company_info', 'job_opportunity', 'company_contact_info', 'portfolio_request', 'clients_reviews', 'irrelevant','pricing_request']
...
def create_query_routing_task(user_message: str, intent: str, session_id: str, conversation_context: str):
    ...
    return Task(
        ...
        UPDATED TOOL CALLING INSTRUCTIONS:
        - For ALL tools, pass both the user_message AND conversation_context parameters
        - Use this format for context-aware tools:
          * search_company_info(question=user_message, conversation_context=conversation_context)
          * handle_greeting_feedbacks(user_message=user_message, conversation_context=conversation_context)
          * company_portfolio(user_message=user_message, conversation_context=conversation_context)
          * clients_reviews(user_message=user_message, conversation_context=conversation_context)
          * handle_irrelevant_queries(user_message=user_message, conversation_context=conversation_context)
          * company_contact_info(conversation_context=conversation_context)
          * looking_job_opportunity(conversation_context=conversation_context)
          * pricing_info(user_message=user_message, conversation_context=conversation_context)
        ...
        IMPORTANT:
        - Always pass conversation_context to tools that support it
        - This enables tools to provide more natural, contextually aware responses
        - Return EXACTLY what the chosen tool outputs
        """,
        expected_output="Exact tool output based on intent classification and session state with conversational awareness",
